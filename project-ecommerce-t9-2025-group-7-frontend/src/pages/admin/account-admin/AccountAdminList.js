...
const AccountAdminList = () => {
  ...
  const fetchAccounts = async () => {
    try {
      ...
      const res = await axios.get(`${DOMAIN_SERVER}/admin/account`, {
        params: params
      });
      ...
      await Swal.fire({
        icon: 'error',
        title: 'Lỗi!',
        text: 'Không thể tải danh sách tài khoản quản trị',
  ...
  const handleDelete = async (id, username) => {
    ...
    if (result.isConfirmed) {
      ...
      try {
        const res = await axios.delete(`${DOMAIN_SERVER}/admin/account/${id}`, {
        ...
        if (res.data.success) {
          await Swal.fire({
            icon: 'success',
            title: 'Thành công!',
            text: 'Tài khoản quản trị đã được xóa',
            confirmButtonText: 'OK',
          ...
          await Swal.fire({
            icon: 'error',
            title: 'Lỗi!',
            text: res.data.message || 'Không thể xóa tài khoản quản trị',
            confirmButtonText: 'OK',
            confirmButtonColor: '#dc3545'
          });
        }
        ...
        await Swal.fire({
          icon: 'error',
          title: 'Lỗi!',
          text: error.response?.data?.message || 'Không thể xóa tài khoản quản trị',
          confirmButtonText: 'OK',
          confirmButtonColor: '#dc3545'
        });
      }
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
          <div className="px-6 py-6 border-b border-gray-200">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
              <div>
                <h2 className="text-2xl font-bold text-gray-900">Danh sách tài khoản quản trị</h2>
                <p className="text-gray-600 mt-1">Quản lý các tài khoản quản trị trong hệ thống</p>
              </div>
                    ...
                    <td colSpan="8" className="px-6 py-12 text-center text-gray-500">
                      ...
                      <p className="mt-1 text-sm text-gray-500">
                        {searchTerm || statusFilter !== 'all' 
                          ...
                          : 'Bắt đầu bằng cách tạo tài khoản đầu tiên'
                        }
                      </p>
                    </td>
                  </tr>
