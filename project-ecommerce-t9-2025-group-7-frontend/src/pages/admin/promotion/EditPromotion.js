...
const EditPromotion = () => {
  ...
  const fetchPromotion = async () => {
    try {
      ...
      if (res.data.success && Array.isArray(res.data.data)) {
        const promotion = res.data.data.find(p => p._id === id);
        if (promotion) {
          ...
          await Swal.fire({
            icon: 'error',
            title: 'Lỗi!',
            text: 'Không tìm thấy mã khuyến mãi',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await axios.patch(`${DOMAIN_SERVER}/admin/promotion/${id}/update`, submitData);
      
      if (res.data.success === true) {
        await Swal.fire({
          icon: 'success',
          title: 'Thành công!',
          text: 'Mã khuyến mãi đã được cập nhật thành công',
          confirmButtonText: 'OK',
        ...
        await Swal.fire({
          icon: 'error',
          title: 'Lỗi!',
          text: res.data.message || 'Cập nhật mã khuyến mãi thất bại',
          confirmButtonText: 'OK',
          confirmButtonColor: '#dc3545'
        });
      }
      ...
      await Swal.fire({
        icon: 'error',
        title: 'Lỗi!',
        text: error.response?.data?.message || 'Cập nhật mã khuyến mãi thất bại',
        confirmButtonText: 'OK',
        confirmButtonColor: '#dc3545'
      });
    } finally {
      ...
      <div className="max-w-4xl mx-auto">
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
          <div className="px-6 py-6 border-b border-gray-200">
            <h2 className="text-2xl font-bold text-gray-900">Chỉnh sửa mã khuyến mãi</h2>
            <p className="text-gray-600 mt-1">Cập nhật thông tin mã khuyến mãi</p>
          </div>
          ...
          <form ref={formRef} onSubmit={handleSubmit} className="px-6 py-6 space-y-6">
            ...
            <div className="flex gap-4 pt-6 border-t border-gray-200">
              ...
              >
                {isSubmitting ? 'Đang cập nhật...' : 'Cập nhật mã khuyến mãi'}
              </button>
            </div>
          </form>
        </div>
