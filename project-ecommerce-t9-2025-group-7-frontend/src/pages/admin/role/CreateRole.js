...
const CreateRole = () => {
  ...
  const fetchPermissions = async () => {
    try {
      ...
      await Swal.fire({
        icon: 'error',
        title: 'Lỗi!',
        text: 'Không thể tải danh sách quyền',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await axios.post(`${DOMAIN_SERVER}/admin/role/create`, requestData);
      
      if (res.data.success === true) {
        await Swal.fire({
          icon: 'success',
          title: 'Thành công!',
          text: 'Nhóm quyền đã được thêm mới thành công',
          confirmButtonText: 'OK',
      ...
      await Swal.fire({
        icon: 'error',
        title: 'Lỗi!',
        text: error.response?.data?.message || 'Có lỗi xảy ra khi thêm nhóm quyền. Vui lòng thử lại.',
        confirmButtonText: 'OK',
        confirmButtonColor: '#dc3545'
      });
    } finally {
      ...
      <div className="max-w-4xl mx-auto">
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
          <div className="bg-white px-6 py-8 border-b border-gray-200">
            <h2 className="text-2xl font-bold text-gray-900 mb-2">Thêm mới nhóm quyền</h2>
            <p className="text-gray-600">Nhập thông tin nhóm quyền mới</p>
          </div>
